...
class Application_Model_Tables_User extends Application_Model_Tables_Abstract
{
    ...
    protected function _checkDublicateRows(array $data)
    {
        if ($data['username']) { // prevent the same usernames
            ...
            if (count($rowset) > 0) {
                throw new Application_Model_Tables_Exception(sprintf($this->_messages[self::MESSAGE_FIELD_EXISTS], $data['username']));
            }
        }
    }
